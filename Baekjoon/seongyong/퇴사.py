...
# main logic
consults = read_lines_as(list, int, ' ', int(read_line()))
memo = [0] * (len(consults) + 1)
for now, consulting in enumerate(consults):
    term, pay = consulting
    consult_end = now + term
    if consult_end > len(consults):
        continue
    for after_end in range(consult_end, len(consults) + 1):
        memo[after_end] = max(memo[now] + pay, memo[after_end])

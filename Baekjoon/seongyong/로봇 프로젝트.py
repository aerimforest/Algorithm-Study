...
while True:
    try:
        width = int(read_line()) * (10 ** 7)
        n = int(read_line())
        legos = sorted(read_lines(n, int))
        bigger, smaller = len(legos) - 1, 0
        while bigger > smaller:
            summation = legos[bigger] + legos[smaller]
            if summation == width:
                break
        ...
        if bigger > smaller:
            print(f'yes {legos[smaller]} {legos[bigger]}')
        else:
            print('danger')
    except:
        break

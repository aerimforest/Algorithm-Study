...
# main logic
num_tables, num_people = read_n_map(int, ' ')
judging_times = read_lines(num_tables, int)
low, high = min(judging_times), max(judging_times) * num_people + 1
min_spent = high
while low <=high:
    mid = (low + high) // 2
    num_judged = sum(map(lambda t: mid // t, judging_times))
    if num_judged >= num_people:
        high = mid - 1
        min_spent = min(mid, min_spent)
    else:
        low = mid + 1

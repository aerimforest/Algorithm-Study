...
def mix(target: list):
    ...
    for _ in range(len(target) // 2):
        rear.append(target.pop())

    front = target
    reverse_mixed = list()
    if len(front) > len(rear):
        reverse_mixed.append(front.pop())
    while front:
        reverse_mixed.append(rear.pop())
        reverse_mixed.append(front.pop())

    mixed = list(reversed(reverse_mixed))
    return mixed


x = int(read_line())
current_str = list(read_line())
mixed = copy.deepcopy(current_str)
cycle = 0
while True:
    mixed = mix(mixed)
    cycle += 1

    if mixed == current_str:
        break

mix_num_to_original = cycle - (x % cycle)
if mix_num_to_original is cycle:
    mix_num_to_original = 0
for _ in range(mix_num_to_original):
    mixed = mix(mixed)
